...
async function validateTokens(session_store){
    ...
    console.log("auth_data exists");
    console.log(session_store.auth_data);
    console.log(session_store.user_data);
    ...
    if(Date.now() > at_expire){
        console.log("access token has expired, check the refresh token.");
        //If it's expired, nothing we can do, they gotta log back in
        if(Date.now() > rt_expire) {
            console.log("Refresh token expired, user will need to re-authenticate");
            return Error("Refresh token expired, user will need to re-authenticate");
        }
        console.log("refresh token is still valid, obtain new access/refresh from bungie api.");
...
function saveTokenData(session_store, token_data){
    ...
    console.log("completed saving token data");
    console.log(session_store.user_data);
    return true;
}
